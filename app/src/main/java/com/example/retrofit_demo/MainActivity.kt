...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        button.setOnClickListener {
            ...
            newDestination.email = text_email.text.toString()
            newDestination.password = text_password.toString()
            newDestination.confirmPassword = text_confirm.toString()

            val destinationService: DestinationService = ServiceBuilder.builService(DestinationService::class.java)

            val requestcall = destinationService.addUserdetails(newDestination)

            requestcall.enqueue(object : Callback<Destination>
            {
                override fun onFailure(call: Call<Destination>?, t: Throwable?)
                {
                    Toast.makeText(this@MainActivity, "failed to retrivexx items", Toast.LENGTH_SHORT).show()
                    Log.e("oppo","thorowable"+ t?.localizedMessage)
                }

                override fun onResponse(call: Call<Destination>?, response: Response<Destination>) {
                    if (response.isSuccessful)
                    {
                        Toast.makeText(this@MainActivity, "sucesss", Toast.LENGTH_SHORT).show()
                    }
                    else {
                        Toast.makeText(this@MainActivity, "failed to retrive items",
                            Toast.LENGTH_SHORT
                        ).show()
                    }
                }
...
/* private fun loadDestinations()
    {
       val destinationService:DestinationService=ServiceBuilder.builService(DestinationService::class.java)
        ...
        // (_>call Destinationservice ->RETURN Call objects- fetch destination from server
